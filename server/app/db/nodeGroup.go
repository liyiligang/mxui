...
//新增节点组
func (db *DB) AddNodeGroup(nodeGroup orm.NodeGroup) error {
	return db.Gorm.Create(&nodeGroup).Error
}

//删除节点组
func (db *DB) DelNodeGroup(nodeGroup orm.NodeGroup) error {
	return db.Gorm.Delete(&nodeGroup).Error
}

//获取节点组信息
func (db *DB) FindNodeGroup(filter protoManage.Filter) ([]orm.NodeGroup, error) {
...
//获取节点组计数
func (db *DB) FindNodeGroupCount(filter protoManage.Filter) (int64, error) {
...
//按ID获取指定节点组
func (db *DB) FindNodeGroupByID(nodeGroup orm.NodeGroup) (*orm.NodeGroup, error) {
	err := db.Gorm.Where("id = ?", nodeGroup.ID).First(&nodeGroup).Error
	return &nodeGroup, err
}

//按名称获取指定节点组
func (db *DB) FindNodeGroupByName(nodeGroup orm.NodeGroup) (*orm.NodeGroup, error) {
