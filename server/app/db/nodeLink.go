...
//新增节点连接
func (db *DB) dbAddNodeLink(nodeLink orm.NodeLink) error {
	return db.Gorm.Create(&nodeLink).Error
}

//删除节点连接
func (db *DB) dbDelNodeLink(nodeLink orm.NodeLink) error {
	return db.Gorm.Delete(&nodeLink).Error
}

//按节点ID删除所有节点连接
func (db *DB) dbDelAllNodeLinkByNodeID(nodeLink orm.NodeLink) error {
	return db.Gorm.Where("sourceID = ? or targetID = ?", nodeLink.SourceID, nodeLink.TargetID).Delete(orm.NodeLink{}).Error
}

//按ID更新节点连接状态
func (db *DB) dbUpdateNodeLinkState(nodeLink orm.NodeLink) error {
	return db.Gorm.Model(&nodeLink).Update("state", nodeLink.State).Error
}

//获取节点连接信息
func (db *DB) dbFindNodeLink(filter protoManage.Filter) ([]orm.NodeLink, error) {
...
//获取节点连接计数
func (db *DB) dbFindNodeLinkCount(filter protoManage.Filter) (int64, error) {
...
//获取节点连接源节点状态统计
func (db *DB) dbFindNodeLinkTargetStateCount(filter protoManage.Filter) ([]orm.StateCount, error) {
	tx := db.Gorm.Offset(int(filter.PageSize*filter.PageNum)).Limit(int(filter.PageSize))
	tx = db.dbSetFilter(tx, filter)
...
//获取节点连接目标节点状态统计
func (db *DB) dbFindNodeLinkSourceStateCount(filter protoManage.Filter) ([]orm.StateCount, error) {
	tx := db.Gorm.Offset(int(filter.PageSize*filter.PageNum)).Limit(int(filter.PageSize))
	tx = db.dbSetFilter(tx, filter)
	subQuery1 := tx.Model(&orm.Node{})
	subQuery2 := db.Gorm.Select("t.id").
...
//获取节点连接中节点ID对应的节点信息
func (db *DB) dbFindNodeByNodeLink(filter protoManage.Filter) ([]orm.Node, error) {
	tx := db.Gorm.Offset(int(filter.PageSize*filter.PageNum)).Limit(int(filter.PageSize))
	tx = db.dbSetFilter(tx, filter)
...
//按ID获取指定节点连接
func (db *DB) dbFindNodeLinkByID(nodeLink orm.NodeLink) (*orm.NodeLink, error) {
	err := db.Gorm.First(&nodeLink, nodeLink.ID).Error
	return &nodeLink, err
}

//按节点ID获取指定节点连接
func (db *DB) dbFindNodeLinkByNodeID(nodeLink orm.NodeLink) (*orm.NodeLink, error) {
